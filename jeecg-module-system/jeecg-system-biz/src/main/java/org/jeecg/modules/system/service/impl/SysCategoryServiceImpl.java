...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	public List<String> loadDictItem(String ids, boolean delNotExist) {
		if (!StringUtils.isEmpty(ids)&&ids.length()<=6){
			SysCategory sysCategory = baseMapper.selectOne(new LambdaQueryWrapper<SysCategory>().eq(SysCategory::getCode,ids).select(SysCategory::getId));
			if (!ObjectUtils.isEmpty(sysCategory)) {
				ids=sysCategory.getId();
			}
		}
