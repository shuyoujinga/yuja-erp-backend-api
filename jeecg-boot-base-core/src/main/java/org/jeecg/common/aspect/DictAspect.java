...
public class DictAspect {
    ...
    private Object parseDictText(Object result) {
        ...
        if (data instanceof IPage) {
            records = (List<Object>) ((IPage<?>) data).getRecords();
        } else if (data instanceof List) {
            records = (List<Object>) data;
        } else {
            return result; // 不支持类型直接返回
        }

        if (!checkHasDict(records)) return result;

        List<JSONObject> items = processRecords(records);

        if (data instanceof IPage) {
            IPage<?> pageData = (IPage<?>) data;
            ((IPage<Object>) pageData).setRecords((List<Object>)(List<?>) items);
        } else if (data instanceof List) {
            // List类型直接替换
            // 因为 Result 泛型是 ?, 所以安全做法是直接 setResult
    ...
    private List<JSONObject> processRecords(List<Object> records) {
        ...
        for (Object record : records) {
            JSONObject item;
            try {
                item = JSONObject.parseObject(objectMapper.writeValueAsString(record), Feature.OrderedField);
            ...
            for (Field field : oConvertUtils.getAllFields(record)) {
                ...
                if (dictAnno != null) {
                    if (!dictFieldList.contains(field)) dictFieldList.add(field);

                    String dictCode = dictAnno.dicCode();
                    if (!StringUtils.isEmpty(dictAnno.dictTable())) {
                        dictCode = new StringBuilder()
                                .append(dictAnno.dictTable()).append(",")
                                .append(dictAnno.dicText()).append(",")
                                .append(dictAnno.dicCode()).append(",")
                                .append(dictAnno.ds()).toString();
                    }
        ...
        Map<String, List<DictModel>> translText = translateAllDict(dataListMap);
            ...
            for (Field field : dictFieldList) {
                ...
                if (oConvertUtils.isNotEmpty(value)) {
                    ...
                    if (dictModels != null && !dictModels.isEmpty()) {
                        record.put(field.getName() + CommonConstant.DICT_TEXT_SUFFIX,
                                translDictText(dictModels, value));
                    }
                }
            }
        }
...
//    =========================================暂时不用的方法=====================================
     ...
     * @param result
     */
    private Object parseDictTextOld(Object result) {
