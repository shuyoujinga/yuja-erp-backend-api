...
public class QueryGenerator {
	 ...
	 * <br>错误示例: QueryWrapper<JeecgDemo> queryWrapper = new QueryWrapper<JeecgDemo>(jeecgDemo);
	 * <br>正确示例: QueryWrapper<JeecgDemo> queryWrapper = new QueryWrapper<JeecgDemo>();
	 * <br>3. 也可以直接调用 {@link #initQueryWrapper} 获取实例
	 *
	 * @param queryWrapper 查询包装器
	 * @param searchObj 查询条件对象
	 * @param parameterMap 请求参数映射
	 */
	private static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
		...
		for (PropertyDescriptor descriptor : origDescriptors) {
			...
			try {
				...
				// 处理多选字段模糊查询
				if (value != null && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
